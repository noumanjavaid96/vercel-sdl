...
function formatGitInfo(meta?: Record<string, string>): string {
  if (!meta) return 'N/A';
...
export function formatDeployment(deployment: Deployment, index: number): string {
  ...
  const lines: string[] = [
    ...
    `  ${colors.bright}URL:${colors.reset} ${
      deployment.url ? `https://${deployment.url}` : colors.dim + 'null (incomplete)' + colors.reset
    }`,
    `  ${colors.bright}Status:${colors.reset} ${stateColor}${state}${colors.reset}`,
    `  ${colors.bright}Created:${colors.reset} ${formatTimestamp(
  ...
  if (deployment.target) {
    ...
    lines.push(
      `  ${colors.bright}Environment:${colors.reset} ${targetColor}${deployment.target}${colors.reset}`
    );
  }
  ...
  if (deployment.inspectorUrl) {
    lines.push(
      `  ${colors.bright}Inspector:${colors.reset} ${deployment.inspectorUrl}`
    );
  }
...
export function formatDeploymentsList(deployments: Deployment[]): string {
  ...
  const formattedDeployments = deployments.map((deployment, index) =>

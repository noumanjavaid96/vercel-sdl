...
export class VercelAPIClient {
    ...
    async listDeployments(params = {}) {
        ...
        if (params.target)
            queryParams.set('target', params.target);
        if (params.state)
            queryParams.set('state', params.state);
        if (params.since)
            queryParams.set('since', params.since.toString());
        if (params.until)
            queryParams.set('until', params.until.toString());
        const url = `${VERCEL_API_BASE_URL}/v6/deployments${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
        try {
    ...
    async listFiles(deploymentId, params) {
        const queryParams = new URLSearchParams();
        if (params?.teamId)
            queryParams.set('teamId', params.teamId);
        if (params?.slug)
            queryParams.set('slug', params.slug);
        const url = `${VERCEL_API_BASE_URL}/v6/deployments/${deploymentId}/files${queryParams.toString() ? `?${queryParams}` : ''}`;
        const response = await fetch(url, {
            headers: {
                Authorization: `Bearer ${this.bearerToken}`,
                'Content-Type': 'application/json',
    ...
    /// writing to disk.
    async getFileContents(deploymentId, fileId, params) {
        const queryParams = new URLSearchParams();
        if (params?.teamId)
            queryParams.set('teamId', params.teamId);
        if (params?.slug)
        ...
        const url = `${VERCEL_API_BASE_URL}/v8/deployments/${deploymentId}/files/${fileId}${queryParams.toString() ? `?${queryParams}` : ''}`;
        const response = await fetch(url, {
            headers: {
                Authorization: `Bearer ${this.bearerToken}`,
            },

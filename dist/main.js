...
async function main() {
    ...
    if (!token) {
        console.error(formatError(new Error('Missing required --token parameter. Provide your Vercel API token.')));
        printUsage();
        process.exit(1);
    }
    try {
        ...
        if (result) {
            ...
            if (answer.toLowerCase() === 'y') {
                const outputDir = path.join(process.cwd(), 'downloads', deployment.uid);
                console.log(`\n→ Will download to: ${outputDir}\n`);
                const downloader = new FileDownloader(client, deployment.uid, outputDir, args.team, args.slug);
                const stats = await downloader.downloadAll();
                printSummary(stats.totalFiles, stats.successCount, stats.failedCount, stats.durationMs, outputDir);
            }
            else {
